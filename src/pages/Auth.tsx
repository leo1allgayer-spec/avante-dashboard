...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      // Login simultâneo nas duas instâncias do Supabase (SSO)
      const [metricsResult, clientsResult] = await Promise.all([
        supabase.auth.signInWithPassword({ email, password }),
        supabaseClients.auth.signInWithPassword({ email, password }).catch(err => ({ error: err }))
      ]);

      if (metricsResult.error) {
        toast({ title: "Erro ao entrar", description: metricsResult.error.message, variant: "destructive" });
      }

      if (clientsResult && 'error' in clientsResult && clientsResult.error) {
        console.warn("Aviso de Login SSO (Gestão de Clientes):", clientsResult.error);
      }
    } else {
      const [metricsResult, clientsResult] = await Promise.all([
      ...
      if (metricsResult.error) {
        toast({ title: "Erro ao cadastrar", description: metricsResult.error.message, variant: "destructive" });
      } else {
