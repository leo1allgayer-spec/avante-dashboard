  ...
  useEffect(() => {
    async function confirmReschedule() {
      if (!token) {
        setResult({
          success: false,
          message: "Link de remarcação inválido.",
          booking_id: null,
          student_name: null,
          course_name: null,
          course_date: null,
      ...
      const { data, error } = await supabase.rpc("confirm_public_course_reschedule", {
        p_token: token,
      });
      ...
      if (error) {
        setResult({
          success: false,
          message: "Não foi possível confirmar a remarcação. Tente novamente ou chame a equipe.",
          booking_id: null,
          student_name: null,
          course_name: null,
          course_date: null,
        ...
        const row = (data?.[0] || null) as RescheduleResult | null;
        setResult(row);
          ...
          <CardContent className="space-y-6 p-6 text-center sm:p-8">
            ...
            {loading ? (
              <div className="space-y-4">
                ...
                <div>
                  <h1 className="font-display text-2xl font-bold">Confirmando remarcação...</h1>
              ...
              <div className="space-y-5">
                ...
                <div>
                  <h1 className="font-display text-2xl font-bold">
                    {success ? "Curso remarcado!" : "Não foi possível remarcar"}
                  </h1>
                  ...
                  <div className="rounded-2xl border border-primary/20 bg-primary/10 px-4 py-3 text-left">
                    ...
                    <div className="mt-2 flex flex-wrap gap-3 text-sm text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1.5">
                        ...
                        {formatDate(result?.course_date)} {result?.course_time ? `- ${formatTime(result.course_time)}` : ""}
                      </span>
                    </div>
                  </div>
                )}
