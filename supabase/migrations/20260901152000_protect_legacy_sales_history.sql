...
begin
  ...
  loop
    ...
    where venda.aluno_futuro_id is null
      and lower(coalesce(venda.origem, '')) = 'cadastro do aluno'
    ...
    if v_sale_id is not null then
      update public.vendas set
        ...
        status = case when v_pending <= 0 then 'pago' else 'pendente' end,
      ...
      do update set
        ...
        updated_at=now()
      where lower(coalesce(public.vendas.origem, '')) = 'cadastro do aluno'
    ...
    if v_closing_id is null then
      ...
      where id = v_closing_id
        and lower(coalesce(origem, '')) = 'cadastro do aluno';
    end if;
  end loop;
  return new;
end;
