...
drop function if exists public.create_support_booking(text, date, time);
...
begin
  ...
  if exists (select 1 from public.support_bookings where cpf_limpo = v_cpf and booking_date = p_date and start_time = p_start_time and status <> 'cancelado') then
    raise exception 'Esta aula já está agendada para você';
  end if;
  ...
  insert into public.support_bookings (student_id, cpf_limpo, student_name, student_phone, booking_date, start_time)
  values (v_student.id, v_cpf, v_name, v_phone, p_date, p_start_time)
  returning * into v_booking;

  return jsonb_build_object('id', v_booking.id, 'name', v_booking.student_name, 'date', v_booking.booking_date, 'time', v_booking.start_time, 'used', v_used + 1, 'remaining', greatest(0, 2 - v_used));
end;
$$;

revoke all on function public.create_support_booking(text, date, time, text, text) from public;
grant execute on function public.create_support_booking(text, date, time, text, text) to anon, authenticated;
