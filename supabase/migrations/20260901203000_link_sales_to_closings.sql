...
-- Vincula somente pares historicos inequivocos. Casos ambiguos permanecem sem vinculo
-- para revisao manual e nunca sao excluidos ou mesclados automaticamente.
with candidates as (
  select
    f.id as fechamento_id,
    v.id as venda_id,
    count(*) over (partition by f.id) as sales_for_closing,
    count(*) over (partition by v.id) as closings_for_sale
  from public.fechamentos_diarios f
  join public.vendas v
    on f.user_id = v.user_id
   and lower(trim(f.cliente)) = lower(trim(v.cliente))
   and lower(trim(f.vendedor)) = lower(trim(v.vendedor))
   and lower(trim(coalesce(nullif(f.categoria, ''), f.produto_servico))) =
       lower(trim(coalesce(nullif(v.produto, ''), v.servico)))
  where f.venda_id is null
    and lower(coalesce(f.status, '')) <> 'cancelado'
)
update public.fechamentos_diarios f
set venda_id = c.venda_id,
    updated_at = now()
from candidates c
where f.id = c.fechamento_id
  and c.sales_for_closing = 1
  and c.closings_for_sale = 1;

create unique index if not exists fechamentos_diarios_venda_id_unique
  on public.fechamentos_diarios(venda_id)
  where venda_id is not null;

create or replace view public.auditoria_vinculos_financeiros
...
select
  ...
  v.cliente,
  v.vendedor,
  coalesce(nullif(v.produto, ''), v.servico) as item,
  f.id as fechamento_id,
  case when f.id is null then 'sem fechamento vinculado' else 'vinculado' end as situacao
from public.vendas v
left join public.fechamentos_diarios f on f.venda_id = v.id;
...
begin
  ...
  loop
    ...
    if v_closing_id is null then
      select min(fechamento.id) into v_closing_id
      from public.fechamentos_diarios fechamento
