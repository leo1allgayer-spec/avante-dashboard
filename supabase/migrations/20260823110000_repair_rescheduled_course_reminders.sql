...
begin
  ...
  if new.status <> 'confirmed' or coalesce(new.course_status, '') in ('cancelado', 'concluido', 'concluído') then
    return new;
  end if;
...
for each row execute function public.reschedule_course_whatsapp_messages();
